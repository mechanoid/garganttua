...
export class TaskScheduleElement extends HTMLElement {
  ...
  provideConfigAsCSSProps(): void {
    ...
    if (this.state) {
      this.style.setProperty(
        "background-color",
        `var(--task-state-${this.state}-bg, var(--task-schedule-base-bg-col))`,
      );
      this.style.setProperty(
        "color",
        `var(--task-state-${this.state}-ft, inherit)`,
      );
    }
  }
  ...
  get start(): number {
    if (this._start) {
      ...
      if (this.ganttChart?.start && this.ganttChart?.end) {
        this._start = dateToGridStartCol(
          this.startDate as Date,
          this.ganttChart.start,
          this.ganttChart.columnCount,
        );
        return this._start;
      }
  ...
  set startDate(unparsed: string | Date) {
    this._startDate = (unparsed instanceof Date)
      ? unparsed
      : parseISO(unparsed);
    this.setAttribute(
      "start",
      formatISO(this._startDate, { representation: "date" }),
    );
  }
  ...
  set endDate(unparsed: string | Date) {
    ...
    this.setAttribute(
      "end",
      formatISO(this._endDate, { representation: "date" }),
    );
  }
  ...
  static build(
    task: Task,
    withoutContent: boolean | null = null,
  ): TaskScheduleElement {
    const element = document.createElement(
      "garganttua-task-schedule",
    ) as TaskScheduleElement;
    const copy = Object.assign({}, task, {
      content: withoutContent ? null : task.content,
    });
    element.task = copy;
